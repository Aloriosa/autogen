...
class GroupChat:
    ...
    def select_speaker(self, last_speaker: Agent, selector: ConversableAgent) -> Agent:
        ...
        #print(selected_agent, agents)
        #raise NoEligibleSpeaker(f"groupchat.select_speaker: innerupting the chat.")
    ...
    def _auto_select_speaker(
        ...
        #__agents_names = [aa._name for aa in agents]
        #logger.warning(__agents_names)
    ...
    def _validate_speaker_name(
        ...
        if len(mentions) == 1:
            ...
            if self.select_speaker_auto_verbose:
                iostream.print(
                    colored(
                        f">>>>>>>> Select speaker attempt #{attempt} of {attempt + attempts_left} failed as it did not include any agent names.",
                        "red",
                    ),
                    flush=True,
                )
    ...
    def _process_speaker_selection_result(self, result, last_speaker: ConversableAgent, agents: Optional[List[Agent]]):
        ...
        if len(result.chat_history) > 0:
            ...
            #logger.warning(f"chat history:\n{set([aa['content'] for i, aa in enumerate(result.chat_history[::-1]) if i % 2 == 1 ])}")
            if "[AGENT SELECTED]" in final_message:
                ...
                if len(agents) > 2 and set([aa['content'] for i, aa in enumerate(result.chat_history[::-1]) if i % 2 == 1 ]):
                    raise NoEligibleSpeaker(f"groupchat._process_speaker_selection_result: innerupting the chat.")
    ...
    def run_chat(
        ...
        for i in range(groupchat.max_round):
            ...
            try:
                ...
                # round_robin speaker selection makes first len(agents) turns are generated without conversational history
                if groupchat.speaker_selection_method.lower() == 'round_robin' and i < len(groupchat.agents) + 1: # message #0 is from som agent
                    reply = speaker.generate_reply(sender=self, messages=speaker._oai_messages[self][:2])
                else:
                #print(f"\n\n round {i}, groupchat messages:{speaker._oai_messages[self]}")
